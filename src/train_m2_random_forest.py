...
Flujo:
  ...
  2. Balancear entrenamiento mediante aumento dinámico.
  3. Entrenar Random Forest sin StandardScaler.
  4. Evaluar sobre test sin aumento.
  5. Generar reportes.
...
SEMILLA = 42
NOMBRE_MODELO = "M2 — Random Forest"
...
def calcular_metricas(
    ...
    """
    Calcula las métricas generales del modelo.
    """
    metricas = {
...
def mostrar_resultados(
    ...
    """
    Muestra las métricas y matriz de confusión en consola.
    """
...
def guardar_importancias(
    ...
    """
    Guarda todas las importancias de características
    y muestra las 10 más relevantes.
    """
    ...
    importancia_df = pd.DataFrame({
        "feature_index": indices_ordenados,
        "feature_name": [
            f"feature_{indice}"
    ...
    ruta_importancias = (
        M2_RF_REPORTS_DIR
        / "importancia_features_m2_random_forest.csv"
    )
    ...
    for posicion, fila in importancia_df.head(10).iterrows():
        print(
            f"   {posicion + 1:>2}. "
            f"Feature {int(fila['feature_index']):<3} "
            f"importancia: {fila['importancia']:.6f}"
        )

    print(
        f"   📊 Importancias guardadas en: "
        f"{ruta_importancias}"
    )
...
def guardar_reportes_evaluacion(
    ...
    """
    Guarda los reportes, gráficos y predicciones.
    """
    M2_RF_REPORTS_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    pd.DataFrame(
        ...
        M2_RF_REPORTS_DIR
        / "reporte_clasificacion_m2_random_forest.csv"
    )
    ...
    pd.DataFrame(
        ...
        M2_RF_REPORTS_DIR
        / "confusion_m2_random_forest.csv"
    )
    ...
    save_confusion_matrix(
        ...
        CLASS_NAMES,
        M2_RF_REPORTS_DIR
        / "confusion_m2_random_forest.png",
    )
    ...
    save_normalized_confusion_matrix(
        ...
        CLASS_NAMES,
        M2_RF_REPORTS_DIR
        / "confusion_normalizada_m2_random_forest.png",
    )
    ...
    save_precision_recall_curves(
        ...
        CLASS_NAMES,
        M2_RF_REPORTS_DIR
        / "precision_recall_m2_random_forest.png",
    )
    ...
    predicciones_df.to_csv(
        M2_RF_REPORTS_DIR
        / "predicciones_m2_random_forest.csv",
        index=False,
    )
...
def guardar_resumen_final(
    ...
    """
    Guarda las métricas, tiempos, tamaño e hiperparámetros.
    """
    resumen = {
    ...
    ruta_resultados = (
        M2_RF_REPORTS_DIR
        / "resultados_m2_random_forest.csv"
    )
...
def mostrar_resumen_tiempos(
    ...
    """
    Muestra los tiempos definitivos del proceso.
    """
    print("\n" + "=" * 60)
...
def main() -> None:
    print("=" * 60)
    print(
        "  M2 — Entrenamiento Random Forest "
        "— VineGuard AI"
    )
    print("=" * 60)
    ...
    # ─────────────────────────────────────────────
    # 1. Carga, aumento y extracción de features
    # Random Forest no utiliza StandardScaler.
    # ─────────────────────────────────────────────
    ...
    model_size_mb = (
        RF_MODEL_PATH.stat().st_size
        / (1024 * 1024)
    )

    # El tiempo total incluye todas las etapas
    tiempo_total = (
        time.perf_counter()
        - inicio_total
    )
