...
function getModeLabel(
  ...
  if (at === "consensus" || modelUsed === "consensus")
    return { mode: t("diagnosis.mode.consensus"), model: t("models.consensus") };
  if (at === "best_model" || modelUsed === "best_model" || modelUsed === "H1")
    return { mode: t("diagnosis.mode.best"), model: MODEL_NAMES[modelUsed] || modelUsed };
  if (at === "all" || modelUsed === "all")
    return { mode: t("diagnosis.mode.compareAll"), model: t("models.allModels") };
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-lg">{t("models.bestModel")}</CardTitle>
          </div>
          {bestModel?.criterio_seleccion ? (
            <CardDescription>{bestModel.criterio_seleccion.join("; ")}</CardDescription>
          ) : (
            <CardDescription>{t("admin.bestModelDesc")}</CardDescription>
          )}
        </CardHeader>
        <CardContent>
          {loading && !bestModel ? (
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-8">
              <div>
                ...
                <Badge variant="outline" className="mt-1 text-xs">{t("admin.rankingPosition")}</Badge>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <p className="text-xs text-muted-foreground">{t("common.accuracy")}</p>
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t("common.f1Macro")}</p>
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t("common.mcc")}</p>
              ...
              {bestModel.victorias_significativas_holm != null && (
                <p className="text-sm text-muted-foreground">
                  {t("admin.holmWins").replace(
                    "{count}",
                    String(bestModel.victorias_significativas_holm)
                  )}
                </p>
              )}
      ...
      {isEmpty ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-xl font-medium">{t("common.noData")}</p>
            <p className="text-sm mt-1">{t("admin.noDataHint")}</p>
          </CardContent>
        </Card>
      ) : (
        <>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t("admin.diseaseDistribution")}</CardTitle>
                <CardDescription>{t("admin.diseaseDistributionDesc")}</CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  ...
                  <>
                    ...
                    {distTotal !== totalDiagnostics && (
                      <p className="text-xs text-amber-600 mt-2 text-center">
                        {t("admin.classSumWarning")
                          .replace("{sum}", String(distTotal))
                          .replace("{total}", String(totalDiagnostics))}
                      </p>
                    )}
                  </>
                ) : (
                  <p className="text-muted-foreground text-center py-12">{t("common.noDataShort")}</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t("admin.diagnosticsByDay")}</CardTitle>
                  ...
                  <LineChart
                    data={summary.diagnostics_by_date.map((d) => ({
                      date: new Date(d.date).toLocaleDateString(
                        language === "en" ? "en-US" : language === "pt" ? "pt-BR" : "es",
                        {
                    ...
                    xKey="date"
                    lines={[{ key: "count", color: "#22C55E", name: t("admin.diagnostics") }]}
                  />
                ) : (
                  <p className="text-muted-foreground text-center py-12">{t("common.noDataShort")}</p>
                )}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t("admin.modelRanking")}</CardTitle>
                <CardDescription>{t("admin.rankingDesc")}</CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <Skeleton className="h-64 w-full" />
                  ...
                  <BarChart
                    ...
                    bars={[
                      ...
                      { key: "f1", color: "#166534", name: t("admin.f1Pct") },
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {summary?.cross_validation && summary.cross_validation.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">{t("stats.crossValidation")}</CardTitle>
                  <CardDescription>{t("admin.cvDesc")}</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>{t("stats.nFolds")}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {summary.cross_validation.map((cv: Record<string, string>, i: number) => (
                        <TableRow key={i}>
                          <TableCell className="font-medium">{cv.modelo || t("admin.modelFallback").replace("{index}", String(i + 1))}</TableCell>
                          <TableCell>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t("admin.systemUsers")}</CardTitle>
                <CardDescription>{t("admin.activeAccountsDesc")}</CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <Skeleton className="h-48 w-full" />
                ) : users.length === 0 ? (
                  <p className="text-muted-foreground text-center py-8">{t("common.noDataShort")}</p>
                ) : (
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">{t("admin.admins")}</span>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">{t("admin.activeUsers")}</span>
                      <span className="font-bold text-green-600">{users.filter((u) => u.active).length}</span>
                    </div>
                    <div className="border-t pt-2 mt-2">
                      <p className="text-xs text-muted-foreground mb-2">{t("admin.lastUsers")}</p>
                        ...
                        {users.slice(0, 4).map((u) => (
                          <div key={u.id} className="flex items-center justify-between text-sm">
                            ...
                            <Badge variant={u.role === "admin" ? "default" : "secondary"} className="text-[10px]">
                              {u.role === "admin" ? t("users.roleAdmin") : t("users.roleClient")}
                            </Badge>
                          </div>
                        ))}
                      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t("admin.recentDiagnostics")}</CardTitle>
              <CardDescription>{t("admin.recentDiagnosticsDesc")}</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t("common.model")}</TableHead>
                    <TableHead>{t("admin.prediction")}</TableHead>
                    <TableHead>{t("result.confidence")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {diagnostics.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                        {t("admin.noRecentDiagnostics")}
                      </TableCell>
                    </TableRow>
                  ) : (
                    diagnostics.map((d) => {
                      const modeInfo = getModeLabel(t, d.model_used, d.analysis_type);
                      return (
                        <TableRow key={d.id}>
